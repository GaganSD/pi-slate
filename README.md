...
- `/slate` — density, footer, sidebar width, and message length. No args opens a picker. `/slate width 30` is 30% of the terminal (floor 28 columns). `narrow` is that floor on every screen. `/slate message-length 50` keeps the last 50 transcript messages (default 100; `all` shows everything).
- `/slate bug` — file a GitHub issue from Pi (`gh`), or open https://github.com/GaganSD/pi-slate/issues.
